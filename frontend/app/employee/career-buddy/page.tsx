...
function BuddyLoading() {
  return (
    <section className="grid min-h-[360px] place-items-center rounded-lg border border-[#D9F3F4] bg-white p-8 text-center shadow-[0_8px_32px_rgba(8,124,126,0.08)]">
      <div>
        <Loader2 className="mx-auto h-8 w-8 animate-spin text-[#087C7E]" />
        <p className="mt-4 text-sm font-bold text-[#1A1033]">Loading Career Buddy</p>
...
function BuddyEmpty({ message, onRetry }: { message: string; onRetry: () => void }) {
  return (
    <section className="rounded-lg border border-[#FBCFE8] bg-white p-6 shadow-[0_8px_32px_rgba(8,124,126,0.08)]">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h2 className="text-lg font-bold text-[#1A1033]">Career Buddy needs a route first</h2>
        ...
        <div className="flex flex-col gap-2 sm:flex-row">
          <button
            type="button"
            onClick={onRetry}
            className="inline-flex min-h-11 items-center justify-center rounded-lg border border-[#DDD0F8] bg-white px-4 text-sm font-bold text-[#6B46C1]"
          >
            Retry
          </button>
          <Link
            href={routes.employeeCareerGps}
            className="inline-flex min-h-11 items-center justify-center gap-2 rounded-lg bg-[#087C7E] px-4 text-sm font-bold text-white"
          >
            Open Career GPS
            <ArrowRight size={16} />
          </Link>
...
export default function EmployeeCareerBuddyPage() {
  ...
  return (
    <main className="min-h-screen bg-[#EFFBFC] text-[#1A1033]">
      <EmployeeTopNav initials={initials} name={profileName} />

      <section className="border-b border-[#D9F3F4] bg-white">
        <div className="mx-auto grid max-w-7xl gap-6 px-4 py-8 sm:px-6 lg:grid-cols-[minmax(0,1fr)_360px] lg:px-8">
          <div>
            <p className="inline-flex items-center gap-2 rounded-full bg-[#E6FAFB] px-3 py-1 text-xs font-bold uppercase tracking-wide text-[#087C7E]">
              <Bot size={14} />
              Career Buddy
            </p>
          ...
          <div className="grid gap-3">
            {[
              ...
              return (
                <div key={item.label} className="rounded-lg border border-[#D9F3F4] bg-[#F7FEFF] p-4">
                  <div className="flex items-center gap-3">
                    <span className="flex h-10 w-10 items-center justify-center rounded-lg bg-white text-[#087C7E]">
                      <Icon size={18} />
                    </span>
                    <div className="min-w-0">
                      <p className="text-xs font-bold uppercase tracking-wide text-[#5B8A8C]">{item.label}</p>
                      <p className="mt-1 truncate text-sm font-bold text-[#1A1033]">{item.value}</p>
                    </div>
                  </div>
                </div>
              );
